...
from donut import DonutDataset, JSONParseEvaluator, save_json, load_json
...
def train(config):
    ...
    model_module.model.half()
    model_module.model.to("cuda")
    ...
    dataset = load_dataset(config.dataset_name_or_paths[0], split="test")

    for idx, sample in tqdm(enumerate(dataset), total=len(dataset)):
        ground_truth = json.loads(sample["ground_truth"])
        output = model_module.model.inference(image=sample["image"], prompt=f"<s_dataset>")["predictions"][0]
    ...
    save_json(f"{config.result_path}/output.json", scores)
    
    #model_module.model.save_pretrained(f"{config.result_path}/{config.exp_name}/{config.exp_version}/trained")
